...
void setDialog(Dialog *dialog, Rectangle rectangle, Rectangle button, char *content) {
    ...
    dialog->okButton = (Rectangle) {dialog->container.x + (dialog->container.width - button.width) / 2, dialog->container.y + dialog->container.height - button.height / 2, button.width, button.height};
...
void DrawDialog() {
    ...
    DrawRectangleRec(currentDialog.okButton, (Color) {38, 77, 124, 255});
    ...
    DrawUnicodeText(BtnContent, getCenter(currentDialog.okButton, MeasureTextEx(BtnContent.font, BtnContent.text, 25, 0)), 25, 0, YELLOW);
}